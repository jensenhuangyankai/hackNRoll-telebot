...
Need help? Type /help for more info on commands!
	...
	if message.chat.type == 'private':
		...
		bot.reply_to(message, reply_message, reply_markup=types.InlineKeyboardMarkup().add(
				types.InlineKeyboardButton('Create event', web_app=types.WebAppInfo(url="https://kaungzinye.github.io/hacknroll-frontend/"))
			))
...
def ask_availability(tele_id, event_id):
	...
	url = create_web_app_url("https://linedoestrolling.github.io/webapp2/", data=data)
	bot.send_message(tele_id, text, markup=types.InlineKeyboardMarkup().add(
				types.InlineKeyboardButton('Set availability', web_app=types.WebAppInfo(url=url))
			))
